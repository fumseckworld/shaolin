		...
		class Git
		{
			...
			public function commits_by_month(string $author): Collect
			{
				...
				do
				{
					...
					if ($i == 26)
						$this->execute("git log --after=$now --before={$month->get($i)} --pretty=format:'%s' --author='$author'"); else
			...
			public function directories(string $directory = '', string $branch = 'master'): array
			{
				...
				if (def($directory))
					$this->execute("git ls-tree --name-only -d $branch  -r $directory"); else
				...
				if (not_def($file))
				{
					...
					foreach ($directories as $k => $v)
					{
						...
						if (def($current_directory))
							append($data, '<tr><td> <a href="' . base_url($this->owner(), $this->repository(), $branch, 'tree', $current_directory, $v) . '"><i class="material-icons">folder</i> ' . $v . '</a></td><td>' . shell_exec("git log $branch --pretty=format:'%s' -n1 -- $current_directory/$v") . '</td><td>' . shell_exec("git log $branch --pretty=format:'%ar' -n1 -- $current_directory/$v") . '</td></tr>'); else
					...
					foreach ($files as $file)
						...
						if (def($current_directory))
							append($data, '<tr><td> <a href="' . base_url($this->owner(), $this->repository(), $branch, 'file', $current_directory, $file) . '"><i class="material-icons">insert_drive_file</i> ' . $file . '</a></td><td>' . shell_exec("git log $branch --pretty=format:'%s' -n1 -- $current_directory/$file") . '</td><td>' . shell_exec("git log $branch --pretty=format:'%ar' -n1 -- $current_directory/$file") . '</td></tr>'); else
			...
			public function git(string $tree, string $file = '', string $branch = 'master'): string
			{
				...
				if (app()->auth()->connected())
					$code = '<div class="text-center"><div class="row"><div class="column"><div class="mb-3"><a class="btn-hollow mr-4" title="Home" href="' . root() . '"><i class="material-icons">group</i></a><a class="btn-hollow mr-4" title="Home" href="' . route('home') . '"><i class="material-icons">person</i></a><a href="' . route('logout') . '" class="btn-hollow mr-4" title="Logout"><i class="material-icons">power_settings_new</i></a><a class="btn-hollow mr-4" title="Download the latest version for your system" href="' . route('download', [$this->repository(), $this->owner()]) . '"><i class="material-icons">get_app</i></a><a class="btn-hollow mr-4" title="Star the project" href="' . route('stars', [$this->repository(), $this->owner()]) . '"><i class="material-icons">star</i></a><a href="#" class="btn-hollow" id="report-bugs"><i class="material-icons">bug_report</i></a></div></div></div></div>'; else
