		...
		class View
		{
			...
			public function __construct(string $views_path, array $config)
			{
				...
				$functions->set(new TwigFunction('display',
					...
					}, ['is_safe' => ['html']]), new TwigFunction('css',
					...
					{
						return copyright();
					}, ['is_safe' => ['html']]), new TwigFunction('mobile',

					function ()
					{
						return is_mobile();
					}, ['is_safe' => ['html']]), new TwigFunction('os',

					function ()
					{
						return new Os();
					}, ['is_safe' => ['html']]), new TwigFunction('bootswatch',

					function (string $theme)
					{
						return bootswatch($theme);
					}, ['is_safe' => ['html']]), new TwigFunction('back',

					function ()
					{
						return url();
					}, ['is_safe' => ['html']]), new TwigFunction('csrf_field',

					function ()
					{
						return csrf_field();
					}, ['is_safe' => ['html']]), new TwigFunction('js',

					function (string $name, string $type = '')
					{
						return js($name, $type);
					}, ['is_safe' => ['html']]), new TwigFunction('img',

					function (string $name, string $alt)
					{
						return img($name, $alt);
					}, ['is_safe' => ['html']]), new TwigFunction('lang',

					function ()
					{
						return app()->lang();
					}, ['is_safe' => ['html']]), new TwigFunction('_',

					function (string $message)
					{
						return gettext($message);
					}, ['is_safe' => ['html']]), new TwigFunction('print',

					function (string $code)
					{
						return $code;
					}, ['is_safe' => ['html']]), new TwigFunction('root',

					function ()
					{
						return root();
					}, ['is_safe' => ['html']]), new TwigFunction('route',

					function (string $name, ...$args)
					{
						return route($name, $args);
					}, ['is_safe' => ['html']]), new TwigFunction('navbar',


					function (string $app_name, string ...$names)
					{
						return navbar($app_name, $names);
					}, ['is_safe' => ['html']]), new TwigFunction('logged',

					function ()
					{
						return app()->auth()->connected();
					}, ['is_safe' => ['html']]), new TwigFunction('user',

					function ()
					{
						return current_user();
					}, ['is_safe' => ['html']]), new TwigFunction('development',
