		...
		class Form
		{
			...
			public function start(string $route, string $confirm = '', string $class = '', bool $enctype = false, string $charset = 'utf-8'): Form
			{
				...
				if ($this->validate)
				{
					...
					{
						if (not_def($class))
							append($this->form, '<form action="' . route($route) . '" method="' . $method . '" class="' . self::VALIDATE . '" accept-charset="' . $charset . '"  enctype="multipart/form-data" onsubmit="return confirm(' . "'" . $confirm . "'" . ')" >'); else
					...
					{
						if (not_def($class))
							append($this->form, '<form action="' . route($route) . '" method="' . $method . '" class="' . self::VALIDATE . '" accept-charset="' . $charset . '"  onsubmit="return confirm(' . "'" . $confirm . "'" . ')">'); else
				...
				} else
				{
					...
					{
						if (not_def($class))
							append($this->form, '<form action="' . route($route) . '" method="' . $method . '" accept-charset="' . $charset . '" enctype="multipart/form-data">'); else
							append($this->form, '<form action="' . route($route) . '" method="' . $method . '" accept-charset="' . $charset . '" class="' . $class . '" enctype="multipart/form-data">');
					} else
					{
						if (not_def($class))
							append($this->form, '<form action="' . route($route) . '" method="' . $method . '" accept-charset="' . $charset . '" >'); else
			...
			{
				if (not_def($ico))
					append($this->form, '<div class="' . self::AUTO_COL . '"><div class="' . $this->separator() . '"><div class="custom-file"><input type="file"  name="' . $name . '" class="custom-file-input ' . $this->get_input_class() . '"   lang="' . $locale . '"><label class="custom-file-label" for="' . $name . '">' . $text . '</label></div></div></div>'); else
			...
			private function generateInput(string $start, string $end, string $input, string $name, string $placeholder, string $value, bool $required, bool $autofocus, bool $autoComplete, string $success_text = '', string $error_text = '')
			{
				...
				if ($this->validate)
				{
					if (not_def($error_text, $success_text))
						throw new Kedavra('missing validation text'); else
				...
				if ($required) // WITH REQUIRED
				{
					...
					if ($autoComplete)
						return '' . $start . ' <input type="' . $input . '" class="' . $class . '" required="required"  placeholder="' . $placeholder . '" name="' . $name . '" value="' . $value . '" autocomplete="on" > ' . $end . $validation . ''; else
			...
			public function textarea(string $name, string $placeholder, string $validation_success_text = '', string $validation_error_text = '', bool $autofocus = false, string $value = ''): Form
			{
				...
				if ($autofocus)
					append($this->form, ' <div class="' . self::AUTO_COL . '">   <div class="' . $this->separator() . '"><textarea rows="' . $row . '"  cols="' . $col . '" placeholder="' . $placeholder . '" autofocus="autofocus" class="' . $class . '" required="required" name="' . $name . '" >' . $value . '</textarea>' . $validation . '</div></div>'); else
			...
			public function submit(string $text, string $icon = '', string $function = ''): Form
			{
				...
				$x = "$function()";
				if (def($function))
					append($this->form, '<div class="' . self::AUTO_COL . '">  <div class="' . $this->separator() . '"><button onclick="' . $x . '" type="submit" class="' . $this->get_btn_class() . ' ' . $class . '">' . $icon . ' ' . $text . '</button></div></div>'); else
			...
			public function select(bool $use_index, string $name, array $options, string $icon = '', string $success_text = '', string $error_text = '', bool $multiple = false, bool $required = true): Form
			{
				...
				if ($this->validate)
					$validation = $this->valid($success_text, $error_text); else
					$validation = '';
				...
				if ($required)
				{
					...
					if (not_def($icon))
					{
						...
						if ($multiple)
							append($this->form, '<div class="' . self::AUTO_COL . '"><div class="' . $this->separator() . '"><select class="' . $class . '"  name="' . $name . '" multiple required="required">'); else
					...
					} else
					{
						...
						if ($multiple)
							append($this->form, '<div class="' . self::AUTO_COL . '"><div class="' . $this->separator() . '"><div class="input-group"><div class="input-group-prepend"><span class="input-group-text">' . $icon . '</span></div> <select name="' . $name . '" class="' . $class . '" multiple required="required">'); else
				...
				} else
				{
					...
					{
						if ($multiple)
							append($this->form, '<div class="' . self::AUTO_COL . '"><div  class="' . $this->separator() . '"><select class="' . $class . '"  name="' . $name . '" multiple>'); else
					...
					} else
					{
						...
						if ($multiple)
							append($this->form, '<div class="' . self::AUTO_COL . '"><div class="' . $this->separator() . '"><div class="input-group"><div class="input-group-prepend"><span class="input-group-text">' . $icon . '</span></div> <select name="' . $name . '" class="' . $class . '" multiple>'); else
			...
			public function checkbox(string $name, string $text, bool $checked = false): Form
			{
				...
				if ($checked)
					append($this->form, '<div class="' . self::AUTO_COL . '"><div class="' . $this->separator() . '"> <div class="custom-control custom-checkbox"><input type="checkbox"  checked="checked" class="custom-control-input ' . $class . '" id="' . $name . '" name="' . $name . '"><label class="custom-control-label" for="' . $name . '">' . $text . '</label></div> </div></div> '); else
			...
			{
				if (def($icon))
					append($this->form, '<div class="' . self::AUTO_COL . '"><div class="' . $this->separator() . '"><div class="input-group"><div class="input-group-prepend"><div class="input-group-text">' . $icon . '</div></div>'); else
				...
				if (def($icon))
					append($this->form, '</select></div></div></div>'); else
			...
			public function generate(int $form_grid, string $table, string $submit_text, string $submit_icon = '', int $mode = Form::CREATE, int $id = 0): string
			{
				...
				if (equal($mode, Form::EDIT))
				{
					...
					foreach ($text->all() as $k => $t)
					{
						...
						{
							if (is_pair($all_text))
								$this->textarea($k, $k, '', '', false, $value); else
					...
					foreach ($numeric->all() as $k => $n)
					{
						...
						{
							if (is_pair($all_num))
								$this->input(Form::NUMBER, $k, $k, '', '', '', $value); else
					...
					foreach ($date->all() as $k => $n)
					{
						...
						{
							if (is_pair($all_date))
								$this->input(Form::DATETIME, $k, $k, '', '', '', $value); else
				...
				} else
				{
					...
					foreach ($text as $k => $t)
					{
						...
						{
							if (is_pair($all_text))
								$this->textarea($t, $t); else
								$this->textarea($t, $t)->end_row_and_new();
						} else
						{
							if (equal($k % 3, 0))
								$this->textarea($t, $t)->end_row_and_new(); else
								$this->textarea($t, $t);
						}
					}
					...
					foreach ($numeric as $k => $n)
					{
						if (different($n, $primary))
						{
							...
							{
								if (is_pair($all_num))
									$this->input(Form::NUMBER, $n, $n); else
					...
					foreach ($date as $k => $d)
					{
						...
						{
							if (is_pair($all_date))
								$this->input(Form::DATE, $d, $d, '', '', '', $current); else
								$this->input(Form::DATE, $d, $d, '', '', '', $current)->end_row_and_new();
						} else
						{
							if (equal($k % 3, 0))
								$this->input(Form::DATE, $d, $d, '', '', '', $current)->end_row_and_new(); else
